...
namespace compiler.CodeAnalysis.Binding
{
    internal abstract class BoundTreeRewriter
    {
        ...
        public virtual BoundExpression RewriteExpression(BoundExpression node)
        {
            switch (node.Kind)
            {
                ...
                case BoundNodeKind.AssigmentExpression:
                    return RewriteAssignmentExpression((BoundAssigmentExpression)node);
        ...
        protected virtual BoundExpression RewriteAssignmentExpression(BoundAssigmentExpression node)
        {
            var expression = RewriteExpression(node.Expression);
            if (expression == node.Expression)
                return node;

            return new BoundAssigmentExpression(node.Variable, expression);
        }
